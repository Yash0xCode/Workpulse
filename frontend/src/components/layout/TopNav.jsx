...
function TopNav({ title = 'Dashboard', subtitle = 'Overview', user, onLogout }) {
  ...
  const handleToggleNotifications = async () => {
    ...
    if (next) {
      await fetchNotifications({ page: 1, limit: 8 })
    }
  }
      ...
      <div className="top-actions">
        <div className="notif-wrap">
          <button className="icon-btn notif-btn" type="button" aria-label="Notifications" onClick={handleToggleNotifications}>
            ...
            {unreadCount > 0 && <span className="notif-badge">{notificationLabel}</span>}
          </button>
          {showNotifMenu && (
        ...
        <div className="user-menu-wrap">
          ...
          >
            ...
            <span className="profile-name">{user?.fullName || user?.name || 'WorkPulse User'}</span>
            <Icon
              name="chevronDown"
              size={14}
          ...
          {showUserMenu && (
            <div className="user-dropdown" role="menu">
              <div className="user-dropdown-header">
                ...
                <div>
                  <div className="dropdown-name">{user?.fullName || user?.name || 'WorkPulse User'}</div>
